...
namespace SecuringAngularApps.API.Controllers
{
    ...
    public class ProjectsController : Controller
    {
        ...
        public async Task<IActionResult> DeleteProject([FromRoute] int id)
        {
            ...
            var users = _context.UserPermissions.Where(up => up.ProjectId == id).ToList();
            users.ForEach(u => _context.UserPermissions.Remove(u));
        ...
        public async Task<IActionResult> AddMilestone([FromBody] Milestone milestone)
        {
            ...
            if (!await ProjectEditAccessCheck(milestone.ProjectId, true)) return Forbid();
